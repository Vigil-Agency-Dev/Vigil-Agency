...
//
// Wall: VIGIL-side dead-drop. Never touches /api/dead-drop-lumina/*.
...
export function createDeadDropVigilClient({ baseUrl, apiKey } = {}) {
  ...
  async function newDirectivesSince(lastSeenIso, prefix = 'cairn_', folder = 'strategy-from-claude') {
    const files = await listFolder(folder);
    const cutoffMs = lastSeenIso ? new Date(lastSeenIso).getTime() : 0;
    return files.filter((f) => {
      if (prefix && !f.name.startsWith(prefix)) return false;
